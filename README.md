...
In this case, let's add `omniauth` and `omniauth-facebook` to the Gemfile. Due to changes to the `omniauth` gem we need to add one more additional gem to our `Gemfile`: `omniauth-rails_csrf_protection`. Once you've added all three gems, run `bundle install`. If we were so inclined, we could add additional OmniAuth gems to our heart's content, offering login via multiple providers in our app.
